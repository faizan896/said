...
import { saidChain, SAID_CONTRACT_ADDRESS, isContractDeployed, networkLabel } from "@/lib/chain";
...
function friendlyError(err: unknown): string {
  ...
  if (/insufficient funds/i.test(message))
    return `Not enough MON to cover gas on ${networkLabel}.`;
  ...
  if (!isContractDeployed) return `Said isn't deployed to ${networkLabel} yet.`;
  return "Something went wrong recording that. Try again.";
}

/** Shared plumbing for every write flow: connect if needed, switch to the
 * configured Monad network if needed, send the tx, wait for it to confirm,
 * then mirror the resulting event into the index via `indexAfterConfirm`. */
