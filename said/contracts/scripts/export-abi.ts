...
async function main() {
  ...
  // Bytecode too, so the app's /deploy page can deploy the contract straight
  // from a browser wallet — no private key in a .env file anywhere.
  fs.writeFileSync(
    path.join(outDir, "Said.bytecode.json"),
  ...
  console.log(`ABI + bytecode exported to ${outDir}`);
}
