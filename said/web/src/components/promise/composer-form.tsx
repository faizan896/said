...
import { isContractDeployed } from "@/lib/chain";
import { formatDate } from "@/lib/format";

const stageCopy: Record<string, string> = {
  connecting: "connecting wallet…",
  "switching-network": "switching to Monad testnet…",
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      {!isContractDeployed && (
        <div className="rounded-lg border border-broken-dim bg-broken-dim px-4 py-3 text-xs text-broken">
          Said isn&rsquo;t deployed to Monad testnet yet in this environment, so recording will
          fail until a contract address is configured. The rest of the flow below is fully wired
          up and ready.
        </div>
      )}
