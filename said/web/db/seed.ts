// Seeds the database with realistic demo data so the landing feed, explore
...
async function main() {
  ...
  const profiles: [string, string, string][] = [
    [FAIZAN, "faizan", daysAgo(190)],
    [ALEX, "0xalex", daysAgo(240)],
    [MARA, "mara", daysAgo(80)],
    [DEV_JUN, "jun", daysAgo(150)],
    [PRIYA, "priya", daysAgo(60)],
  ];
  for (const [address, username, joined] of profiles) {
    await query(
      "INSERT INTO profiles (address, username, joined_at) VALUES ($1, $2, $3)",
      [address, username, joined]
    );
  }
  ...
  for (const s of seeds) {
    await query(
      `INSERT INTO promises
        (id, creator_address, statement, category, created_at, deadline, status,
         proof_url, proof_note, completed_at, create_tx_hash, complete_tx_hash)
      ...
      [
        ...
        s.createdAt,
        s.deadline,
        s.status === "BROKEN" ? "ACTIVE" : s.status, // BROKEN is derived, never stored
        s.proofUrl ?? null,
        s.proofNote ?? null,
        s.completedAt ?? null,
        fakeTx(`create-${s.id}`),
    ...
    for (let i = 0; i < s.witnessCount; i++) {
      await query(
        `INSERT INTO witnesses (promise_id, witness_address, witnessed_at, tx_hash)
         VALUES ($1,$2,$3,$4)`,
        [s.id, witnessAddr(s.id * 1000 + i), daysAgo(0), fakeTx(`witness-${s.id}-${i}`)]
      );
      witnessTotal++;
    }
  }

  console.log(`Seeded ${seeds.length} promises and ${witnessTotal} witnesses.`);
  await getPool().end();
}
